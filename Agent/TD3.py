...
class TD3(object):
    ...
    def choose_action(self, h_epi, state):
        ...
        if self.pointer > self.var_stable_at:
            ...
            if delta_step > 0:
                self.var = self.var_init + delta_step * (self.var_stable - self.var_init) / (
                            self.var_stable_at - self.var_drop_at)
        if h_epi is None:
    ...
    def learn(self):
        ...
        # 其实没什么用
        self.lr_a = max(self.LR_A_STABLE,
                        self.LR_A * np.power(self.LR_DECAY, ((self.pointer - 3000) / self.LR_DECAY_TIME)))
        self.lr_c = max(self.LR_C_STABLE,
                        self.LR_C * np.power(self.LR_DECAY, ((self.pointer - 3000) / self.LR_DECAY_TIME)))
