...
class System:
    def __init__(self):
        ...
        # self.env = Environment(name='TD3_rewardAgent_bank_TD3_rewardIsSparseDayTillEnd2day', lim_day=100, use_wandb = use_wandb)
        self.env = Environment(name='TD3', lim_day=lim_day, use_wandb=use_wandb)
        if use_wandb:
            swanlab.init(project="TD3_vs_PPO",workspace="wx829",config={
                "random_seed":seed,
                "lim_day":lim_day,
            })
        # self.env = Environment(name='TD3_rewardIsday', lim_day=100, use_wandb = use_wandb)
    ...
    def run(self):
        ...
        for episode in range(10000):
            if self.epiday > 300000 and episode % 100 == 0:
                break
